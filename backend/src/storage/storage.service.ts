  ...
  export class StorageService {
    ...
    async uploadBuffer(
      ...
      const dir = join('/app', 'uploads', 'certificates');
      await mkdir(dir, { recursive: true });
      await writeFile(join(dir, filename), Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer));
      const base = this.config.get('BACKEND_URL', 'http://localhost:3000');
