...
export async function seed(ds: DataSource): Promise<void> {
  ...
  const jsLessons = await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: DOM і події',              type: LessonType.QUIZ,  moduleId: m2.id, orderIndex: 3, durationSec: 0,    isFree: false, contentUrl: null, textContent: '[{"question": "Який метод вибирає елемент по CSS-селектору?", "options": ["getElementById()", "querySelector()", "getElementByClass()", "findElement()"], "correctIndex": 1, "explanation": "querySelector() приймає будь-який CSS-селектор"}, {"question": "Яка подія спрацьовує при натисканні миші?", "options": ["hover", "focus", "click", "change"], "correctIndex": 2}, {"question": "Як зупинити спливання події?", "options": ["event.stop()", "event.prevent()", "event.stopPropagation()", "event.cancel()"], "correctIndex": 2}, {"question": "Що повертає querySelector якщо елемент не знайдено?", "options": ["undefined", "false", "null", "0"], "correctIndex": 2}]' }),
  ...
  const sqlLessons = await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: JOIN і агрегація',          type: LessonType.QUIZ,  moduleId: m5.id, orderIndex: 3, durationSec: 0,    isFree: false, contentUrl: null, textContent: '[{"question": "Який JOIN повертає всі рядки лівої таблиці навіть без співпадінь?", "options": ["INNER JOIN", "RIGHT JOIN", "LEFT JOIN", "FULL JOIN"], "correctIndex": 2}, {"question": "Яка функція рахує кількість рядків?", "options": ["SUM()", "COUNT()", "TOTAL()", "NUM()"], "correctIndex": 1}, {"question": "Яке ключове слово використовується для групування?", "options": ["ORDER BY", "WHERE", "GROUP BY", "HAVING"], "correctIndex": 2}, {"question": "Чим HAVING відрізняється від WHERE?", "options": ["Нічим", "HAVING фільтрує до групування", "HAVING фільтрує після GROUP BY", "WHERE не працює з JOIN"], "correctIndex": 2}]' }),
  ...
  const algoLessons = await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: оцінка складності',         type: LessonType.QUIZ,  moduleId: m7a.id, orderIndex: 2, durationSec: 0,    isFree: false, contentUrl: null, textContent: '[{"question": "Яка складність лінійного пошуку?", "options": ["O(1)", "O(log n)", "O(n)", "O(n2)"], "correctIndex": 2}, {"question": "Яка складність бінарного пошуку?", "options": ["O(1)", "O(log n)", "O(n)", "O(n log n)"], "correctIndex": 1}, {"question": "Яка складність bubble sort у гіршому випадку?", "options": ["O(n)", "O(n log n)", "O(n2)", "O(log n)"], "correctIndex": 2}, {"question": "Яка операція у Hash Map має складність O(1)?", "options": ["Сортування", "Пошук за ключем", "Обхід всіх елементів", "Злиття двох map"], "correctIndex": 1}]' }),
  ...
  await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: типи та інтерфейси',   type: LessonType.QUIZ,  moduleId: m8.id, orderIndex: 2, durationSec: 0,    isFree: false, contentUrl: null, textContent: '[{"question": "Чим interface відрізняється від type?", "options": ["Нічим", "interface підтримує extends і декларативне злиття", "interface не підтримує union", "type не можна для обєктів"], "correctIndex": 1}, {"question": "Що таке union тип?", "options": ["Тип з усіма полями обовязково", "Тип який може бути одним з кількох варіантів", "Тип для масивів", "Тип для функцій"], "correctIndex": 1}, {"question": "Як позначається необовязкове поле?", "options": ["field!: string", "field?: string", "field: string | null", "optional field: string"], "correctIndex": 1}, {"question": "Що робить readonly?", "options": ["Робить поле приватним", "Забороняє зміну після ініціалізації", "Робить поле обовязковим", "Забороняє читання"], "correctIndex": 1}]' }),
  ...
  await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: основи ML',                   type: LessonType.QUIZ,  moduleId: m13.id, orderIndex: 2, durationSec: 0,    isFree: false, contentUrl: null, textContent: '[{"question": "Що таке supervised learning?", "options": ["Навчання без даних", "Навчання на розмічених даних", "Навчання через винагороду", "Навчання без учителя"], "correctIndex": 1}, {"question": "Що вимірює loss function?", "options": ["Швидкість навчання", "Кількість параметрів", "Різницю між передбаченням і відповіддю", "Розмір датасету"], "correctIndex": 2}, {"question": "Що таке overfitting?", "options": ["Модель навчилась мало", "Модель перенавчилась на тренувальних даних", "Помилка в даних", "Завеликий датасет"], "correctIndex": 1}, {"question": "Який алгоритм для регресії?", "options": ["K-means", "Лінійна регресія", "PCA", "DBSCAN"], "correctIndex": 1}]' }),
  ]);
  ...
  await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: docker-compose',           type: LessonType.QUIZ,  moduleId: m15.id, orderIndex: 2, durationSec: 0,   isFree: false, contentUrl: null, textContent: '[{"question": "Для чого docker-compose?", "options": ["Для створення образів", "Для запуску кількох контейнерів", "Для деплою в K8s", "Для моніторингу"], "correctIndex": 1}, {"question": "Яка команда запускає сервіси?", "options": ["docker-compose run", "docker-compose build", "docker-compose up", "docker-compose start"], "correctIndex": 2}, {"question": "Що таке volume?", "options": ["Мережа між контейнерами", "Постійне сховище даних", "Змінна середовища", "Порт"], "correctIndex": 1}, {"question": "Як зупинити і видалити контейнери?", "options": ["docker-compose stop", "docker-compose kill", "docker-compose down", "docker-compose remove"], "correctIndex": 2}]' }),
  ...
  await ds.getRepository(Lesson).save([
    ...
    mkLesson(ds, { title: 'Тест: UX принципи',               type: LessonType.QUIZ,  moduleId: m18.id, orderIndex: 2, durationSec: 0,   isFree: false, contentUrl: null, textContent: '[{"question": "Що таке wireframe?", "options": ["Готовий дизайн", "Схематичний макет без деталей", "Анімація переходів", "Фінальний прототип"], "correctIndex": 1}, {"question": "Що означає закон Хікса?", "options": ["Менше варіантів — повільніший вибір", "Більше варіантів — довший вибір", "Юзери читають зліва направо", "Мобайл важливіший"], "correctIndex": 1}, {"question": "Що таке affordance?", "options": ["Кольорова схема", "Властивість елемента що підказує як ним користуватись", "Розмір шрифту", "Анімація кнопки"], "correctIndex": 1}, {"question": "Що таке thumb zone?", "options": ["Правило третин", "Зона зручна для великого пальця на мобільному", "Золотий перетин", "F-паттерн"], "correctIndex": 1}]' }),
